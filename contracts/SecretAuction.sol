...
contract SecretAuction is Ownable {
    ...
    uint256 public currentLeaderBid;
    ...
    modifier StartStage() {
        require(
            auctionStage == AuctionStage.Start,
            "The auction is not in start stage"
        );
        _;
    }
    ...
    modifier CommitStage() {
        require(
            ...
            "The auction is not in commit stage"
        );
        _;
    }
    ...
    modifier RevealStage() {
        require(
            ...
            "The auction is not in reveal stage"
        );
        _;
    }
    ...
    /// @dev allows the caller to access the length of the Leaders array
    function getLeadersNum() public view returns (uint256) {
        return currentLeaders.length;
    }

    /// @dev allows the caller to access a particular element of the Leaders array
    function getLeader(uint256 i) public view returns (address) {
        require(i < currentLeaders.length, "Array index overflow");
        return currentLeaders[i];
    }
    ...
    function RevealCommittedBid(uint256 _bidAmount, string memory _auctionKey)
        public
        RevealStage
    {
        require(
            CommitHashes[msg.sender].length > 0,
            "The user has not committed a bid. Reveal not possible"
        );
        bytes32 revealHash = keccak256(abi.encode(_bidAmount, _auctionKey));

        require(
            CommitHashes[msg.sender] == revealHash,
            "Revealed bid is not the same as the one committed. Reveal not Allowed"
        );
